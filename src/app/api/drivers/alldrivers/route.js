...
export async function POST(req) {
    try {
      ...
      return NextResponse.json({ message: 'Trip info inserted successfully', data }, { status: 200 });
    } catch (error) {
...
//this the api to update the the data base in the code 
  export async function PUT(req) {
    try {
      ...
      return NextResponse.json({ message: 'Trip info updated successfully', data }, { status: 200 });
    } catch (error) {
      return NextResponse.json({ error: error.message }, { status: 500 });
    } 
  }
